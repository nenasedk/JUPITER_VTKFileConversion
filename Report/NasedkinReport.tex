...
\section{Paraview Visualisation}
\subsection{Paraview}
...
Once a VTK file has been produced by the conversion tool, it can be opened in Paraview. 
The user may then select a suitable colour map for the data. 
...
The luminosity should be consistent across the colour range, so as not to emphasise certain features, unless these are the features in the data that should be emphasised. 
...
\item[]\textbf{Clip}
Given an orientation and position, remove all cells `outside' of the cut.
...
Compute particle trajectories in a flow and visualise using streamlines. The user should set an initial point, with a small radius, and select whether to integrate forwards or backwards from that point (or both). For speed, an RK2 integrator is suggested. Typically only a small number of streamlines are necessary to visualise the flow.
...
\begin{figure}
	...
	\caption{\label{fig:dens} Cross section of gas density of the circumplanetary disk of a 1 Jupiter mass planet embedded at 50 AU in the circumstellar disk of a 1 solar mass star.}
...
\begin{figure*}[t]
	\centering
	\includegraphics[width=0.95\linewidth]{figures/DensityVelPlanetCentered5MJ001AU_2.png}
	\caption{\label{fig:vel}Velocity vectors spiralling out from the circumplanetary disk.}
\end{figure*}

With the conversion tool developed, we can present several examples of possible visualisations made using Paraview \cite{para}. Fig. \ref{fig:dens} shows a cross section of the gas density in the region of the circumplanetary disk along with the 3D density structure in Fig. \ref{fig:d3d}. This shows the dens regions near to the forming planet, and a potential location for satellite formation. Fig. \ref{fig:temp} shows the gas temperature of the mid-plane of the disk, coloured to a blackbody spectrum.
...
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{figures/260/GasVelTube.png}
	\caption{\label{fig:tube} Inflow of gas from above onto an accreting planet.}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{figures/TempPlotCircumplanetary5MJ001AU_4.png}
	\caption{\label{fig:shock} Shock front of gas flow near to the planet.}
\end{figure}
\begin{figure}
	\centering
...
\begin{figure}[t]
	\centering
	\includegraphics[width=\linewidth]{figures/GasEnergy1.png}
